    ...
    private void loadMemberData() {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS)) {
            ...
            if (rs.next()) {
                ...
                // Show message if present (set by admin alert)
                String msg = rs.getString("message");
                if (msg != null && !msg.trim().isEmpty()) {
                    statusLabel.setText(msg);
                    statusLabel.setForeground(Color.RED);

                    // Reset message in DB so it only shows once
                    try (PreparedStatement pstClear = conn.prepareStatement("UPDATE members SET message='' WHERE id=?")) {
                        pstClear.setInt(1, memberId);
                        pstClear.executeUpdate();
                    } catch (SQLException e2) {
                        // Ignore, just fail silently
                    }
                } else {
                    statusLabel.setText("");
                    statusLabel.setForeground(new Color(70, 70, 70));
